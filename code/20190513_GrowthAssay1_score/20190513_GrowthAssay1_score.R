...
## Read data
dirs <- "raw_data/20190513_GrowthAssay1_score"
raw <- read_data(dirs)
score <- raw[[1]]
...
# Create datafrome holding info for each worm in each well
plate <- raw_data %>%
  filter(call50 == "object" & !condition =="Wash") %>%
  filter(TOF<1000 & TOF > 90) %>%
  separate(condition, into = c("timepoint","replicate"), sep="_")
write.csv(plate, "processed_data/20190513_GrowthAssay1_score/20190513_GrowthAssay1_Processed.csv", row.names = F, quote = FALSE)
...
summedraw <- sumplate(raw, directories = FALSE, quantiles = TRUE) %>%
  ...
  separate(condition, into = c("timepoint","replicate"), sep="_")
write.csv(summedraw, "processed_data/20190513_GrowthAssay1_score/20190513_GrowthAssay1_Summarized.csv", row.names = F, quote = FALSE)
...
ggsave(plt1, filename = paste("plots_tables/20190513_GrowthAssay1_score/AllData_Density.png",sep=""), height = 8, width = 12)
...
plt2 <- ggplot(plate, aes(x = timepoint, y = TOF))+geom_jitter() + 
  ...
  theme(text = element_text(size=20))
ggsave(plt2, filename = paste("plots_tables/20190513_GrowthAssay1_score/AllData_Scatter.png",sep=""), height = 8, width = 12)
...
plt3 <- ggplot(plate, aes(x = timepoint, y = TOF))+geom_boxplot() + 
  labs(x="Hours",y=expression(paste("Length (", mu, "m)"))) +
  theme(text = element_text(size=20))
ggsave(plt3, filename = paste("plots_tables/20190513_GrowthAssay1_score/AllData_Box.png",sep=""), height = 8, width = 12)
...
ggsave(plt4, filename = paste("plots_tables/20190513_GrowthAssay1_score/AllData_Box.png",sep=""), height = 8, width = 12)
...
ggsave(plt5, filename = paste("plots_tables/20190513_GrowthAssay1_score/Summarized_meanTOFbywell.png",sep=""), height = 8, width = 12)
...
ggsave(plt6, filename = paste("plots_tables/20190513_GrowthAssay1_score/Summarized_meanTOF_by_Technicalrep.png",sep=""), height = 8, width = 12)
...
ggsave(plt7, filename = paste("plots_tables/20190513_GrowthAssay1_score/Summarized_meanTOF_by_Biologicalrep.png",sep=""), height = 8, width = 12)
...
ggsave(plt8, filename = paste("plots_tables/20190513_GrowthAssay1_score/Number_of_Objects.png",sep=""), height = 8, width = 12)
